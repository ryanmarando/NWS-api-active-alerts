...
function changeTimeOutput(singleAlertTime) {
  ...
  let dayPart = "AM";
  let newDateHour = dateTime.getHours();
  if (newDateHour > 12) {
  ...
  let newDateMinutes = dateTime.getMinutes().toString();
  if (newDateMinutes === 0 || newDateMinutes.length === 1)
    newDateMinutes = newDateMinutes + "0";
  let newDateTime = newDateHour + ":" + newDateMinutes + dayPart;
  return newDateTime;
}
...
export function printAllAlerts(alertList) {
  console.log("\n-------------------------------------------------------");
...
function readInData(stateId) {
  const url = "http://api.weather.gov/alerts/active?area=" + stateId;
...
export async function getActiveAlerts(stateId) {
  const results = await readInData(stateId);
  const countiesAdded = countyList.size > 0;
  for (const [idx, alert] of results["features"].entries()) {
    let warningProperties = alert["properties"];
    let singleAlert = createAlert(
